class gen_algo:
    def __init__(self):
        self.nb_run = 1    # nb de run par bot
        self.nb_move = 200  # nb de move par run
        self.nb_start_pop = 400  # nb de bot dans la pop de depart
        self.list_bot = []

    def gen_start_pop(self):
        # generation de la pop de depart
        for i in range(0, self.nb_start_pop):
    ...
    def start(self):
        ...
        for i in range(1, 4000000):
            ...
            for bot in tqdm(list_bot):
                ...
                bot.genes = model.get_weight()
                for piece_set in pieces_set:
                    game_run(bot, model, nb_move=nb_move,
                             piece_set=copy.deepcopy(piece_set))
            ...
            list_bot.sort(key=lambda x: (x.get_lines_cleared(),
                                         x.get_fitness()), reverse=True)
            new_list_bot = []
            ...
            if i > 1:
                ...
                game_run(new_list_bot[0], model, draw_enable=draw_enable,
                         nb_move=100, piece_set=copy.copy(pieces_set[0]))
                print("resultat du meilleur: ",
                      new_list_bot[0].get_lines_cleared(), " lignes cleared")
